...
map.getView().fit([1055425.879824, 5749868.025383, 1063434.025361, 5755754.229615], map.getSize());
...
function onPointerMove(evt) {
    ...
    // Increase hitTolerance so that narrow geometries such as lines are easier to interact with.
        ...
        if (currentFeature !== highlight) {
            ...
            if (currentFeature) {
                ...
                if (currentFeature.getGeometry().getType() == 'Point' || currentFeature.getGeometry().getType() == 'MultiPoint') {
                    ...
                    // Customize the highlight style for line features.  When the user hovers over
                    // the demand‑responsive transport routes (layer "Percorso"), emphasise the
                    // segment by increasing its width and using the requested colour.  For
                    // all other interactive line layers, fall back to the default yellow
                    // highlight using the original stroke width.

                    var featureWidth = featureStyle.getStroke().getWidth();
                    // Determine whether the current layer corresponds to the DRT network
                    // defined in layers.js (popuplayertitle set to "Percorso").  If so,
                    // apply a custom highlight style: width 8 pixels and colour #bd5423.
                    if (currentLayer && currentLayer.get('popuplayertitle') === 'Percorso') {
                        highlightStyle = new ol.style.Style({
                            stroke: new ol.style.Stroke({
                                color: '#bd5423',
                                lineDash: null,
...
var Title = new ol.control.Control({
    element: (() => {
        ...
        titleElement.innerHTML = '<h2 class="project-title">Progetto "Comunità 4x4"</h2>';
        return titleElement;
    })(),
    ...
    element: (() => {
        ...
        if (337 > 240) {
            ...
            window.showAbstract = function() {
                ...
                linkElement.innerHTML = 'Questa è una rappresentazione schematica della proposta di servizio di trasporto pubblico a chiamata nei Comuni di Sant\'Omobono Terme, Rota Imagna, Costa Valle Imagna, Locatello, Corna Imagna, Fuipiano e Brumano. Gli anelli di percorso hanno carattere esemplificativo e non esaustivo delle varie possibilità di connessione intercomunale.';
            }
